import pandas as pd
import pytest
from src.etl.extract.extract_noc_data import (
    extract_noc_data,
    TYPE,
    FILE_PATH,
...
def mock_log_extract_success(mocker):
    return mocker.patch("src.etl.extract.extract_noc_data.log_extract_success")


@pytest.fixture
def mock_logger(mocker):
    return mocker.patch("src.etl.extract.extract_noc_data.logger")
...
def test_extract_data_csv_to_dataframe(mocker):
    ...
    mocker.patch(
        "src.etl.extract.extract_noc_data.pd.read_csv", return_value=mock_df
    )
...
def test_log_extract_success_data(
    ...
    mock_end_time = 100.5
    mocker.patch(
        "src.etl.extract.extract_noc_data.timeit.default_timer",
...
def test_log_data_error(mocker, mock_logger):
    ...
    mocker.patch(
        "src.etl.extract.extract.pd.read_csv",
